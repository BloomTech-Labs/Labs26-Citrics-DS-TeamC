...
@router.post('/humidity_viz/')
async def humidity_viz(user_queried_citystates: list):
    """
    ...
    fig = px.line(subsetW, x='month', y='Humidity', 
                labels={'Humidity': 'Average Daily Humidity (g/kg)', 'month': 'Month'}, 
                color='City_State', title='Average Daily Humidity Grouped by Month').for_each_trace(lambda t: t.update(name=t.name.split("=")[-1]))
