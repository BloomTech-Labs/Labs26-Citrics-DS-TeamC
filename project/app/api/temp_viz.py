...
@router.post('/temp_viz/')
async def temp_viz(user_queried_citystates: list):
    """
    ...
    fig = px.line(subsetW, x='month', y='FeelsLikeF', 
                labels={'FeelsLikeF': 'Average Daily Temperature (\N{DEGREE SIGN}F)', 'month': 'Month'}, 
                color='City_State', title='Average Daily Temperature Grouped by Month').for_each_trace(lambda t: t.update(name=t.name.split("=")[-1]))
