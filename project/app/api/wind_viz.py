...
@router.post('/wind_viz/')
async def wind_viz(user_queried_citystates: list):
    """
    ...
    fig = px.line(subsetW, x='month', y='WindSpeed_kmph', 
                labels={'WindSpeed_kmph': 'Average Daily Wind Speed (km/h)', 'month': 'Month'}, 
                color='City_State', title='Average Daily Wind Speed Grouped by Month').for_each_trace(lambda t: t.update(name=t.name.split("=")[-1]))
