...
async def precip_viz(user_queried_citystates: list):
    ...
    fig = px.line(subsetW, x='month', y='Precip_mm', 
                labels={'Precip_mm': 'Average Daily Precipitation (mm)', 'month': 'Month'}, 
                color='City_State', title='Average Daily Precipitation Grouped by Month').for_each_trace(lambda t: t.update(name=t.name.split("=")[-1]))
