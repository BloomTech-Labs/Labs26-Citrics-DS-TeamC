...
@router.post('/uvindex_viz/')
async def uvindex_viz(user_queried_citystates: list):
    """
    ...
    fig = px.line(subsetW, x='month', y='UVindex', 
                labels={'UVindex': 'Average Daily UV Index', 'month': 'Month'}, 
                color='City_State', title='Average Daily UV Index Grouped by Month').for_each_trace(lambda t: t.update(name=t.name.split("=")[-1]))
